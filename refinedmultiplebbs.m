...
%Initial set up
x0 = LAC_380keV;
x0= imresize3(x0,param.multiFactor);
...
BBs = [
        struct('cx', (midx+DBS)/param.dx , 'cy',(midy -30 )/param.dy, 'cz', (midz + 30)/param.dz);             
        struct('cx', (midx+DBS)/param.dx , 'cy',(midy +0 )/param.dy, 'cz', (midz - 0)/param.dz);   
        struct('cx', (midx+DBS)/param.dx , 'cy',(midy -30+40)/param.dy, 'cz', (midz + 30)/param.dz); 
        struct('cx', (midx+DBS)/param.dx , 'cy',(midy +30 )/param.dy, 'cz', (midz - 30)/param.dz);
        struct('cx', (midx+DBS)/param.dx , 'cy',(midy +40 )/param.dy, 'cz', (midz - 0)/param.dz);   
        struct('cx', (midx+DBS)/param.dx , 'cy',(midy+30+40 )/param.dy, 'cz', (midz - 30)/param.dz);
...
for i = 1:n
    bb_centers(i, 1) = (BBs(i).cx - param.nx/2) * param.dx;
    bb_centers(i, 2) = -(BBs(i).cy - param.ny/2) * param.dy;
    bb_centers(i, 3) = (BBs(i).cz - param.nz/2) * param.dz;
end
...
for i = 1:numel(BBs)
    ...
    BBs_3D(distances <= 2 /param.dx) = tungsten_attenuation_coefficient;
end

x1 = x0 + BBs_3D;

projWithoutBB = CTprojection0(x0,param);
proj = CTprojection2(BBs_3D,param);
maxPixelvalue = max(max(projWithoutBB));

% save proj.mat proj 
    
projCombined = projWithoutBB + proj;
figure;
imshow(projWithoutBB,[]);
figure;
imshow(projCombined, []);

%Binarize projection image with threshold = maxPixelvalue of projWithoutBB
bw =  imbinarize(projCombined,maxPixelvalue);
minSize = 15;
